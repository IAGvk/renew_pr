...
PREFIX = "pvdaq/csv/"  # parquet also available

def main():
    os.makedirs(ROOT, exist_ok=True)
    s3 = boto3.client("s3", config=Config(signature_version=UNSIGNED))
    # Example: grab a small CSV file for demo (PVDAQ has many systems; we pick a small one)
    # List first 100 keys under pvdaq/csv
